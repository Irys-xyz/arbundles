...
const noble_ed25519_1 = tslib_1.__importDefault(require("noble-ed25519"));
...
class Curve25519 {
    ...
    sign(message) {
        return noble_ed25519_1.default.sign(Buffer.from(message), Buffer.from(this.key));
    }
    static async verify(pk, message, signature) {
        let p = pk;
        if (typeof pk === "string")
            p = base64url_1.default.toBuffer(pk);
        return noble_ed25519_1.default.verify(Buffer.from(message), Buffer.from(signature), Buffer.from(p));
    }
}
