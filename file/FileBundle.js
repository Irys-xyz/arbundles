...
class FileBundle {
  ...
  async toTransaction(attributes, arweave, jwk) {
    ...
    const stream = multistream_1.default.obj(streams);
    const tx = await promises_1.pipeline(
      stream,
      arweave_stream_tx_1.createTransactionAsync(attributes, arweave, jwk),
    );
  ...
  async signAndSubmit(arweave, jwk, tags = []) {
    ...
    await promises_1.pipeline(
      stream2,
      arweave_stream_tx_1.uploadTransactionAsync(tx, arweave, true),
    );
    return tx;
  }
  async *getHeaders() {
  ...
  async *itemsGenerator() {
    let counter = 0;
    for await (const { id } of this.getHeaders()) {
      yield new FileDataItem_1.default(
        this.txs[counter],
        base64url_1.default.toBuffer(id),
      );
      counter++;
    }
  }
  async getById(txId) {
    ...
    for await (const { id } of this.getHeaders()) {
      if (id === txId)
        return new FileDataItem_1.default(
          this.txs[counter],
          base64url_1.default.toBuffer(id),
        );
      counter++;
