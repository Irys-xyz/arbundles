...
class DataItem {
  ...
  get signatureType() {
    const signatureTypeVal = utils_1.byteArrayToLong(
  ...
  get rawTags() {
    ...
    const tagsSize = utils_1.byteArrayToLong(
  ...
  get tags() {
    const tagsStart = this.getTagsStart();
    const tagsCount = utils_1.byteArrayToLong(
    ...
    return parser_1.tagsParser.fromBuffer(
  ...
  static async verify(buffer) {
    ...
    const numberOfTags = utils_1.byteArrayToLong(
    ...
    if (numberOfTags > 0) {
      try {
        const tags = parser_1.tagsParser.fromBuffer(
          buffer_1.Buffer.from(
