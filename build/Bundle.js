...
class Bundle {
  ...
  getIds() {
    ...
    for (let i = HEADER_START; i < HEADER_START + 64 * this.length; i += 64) {
      ids.push(
        base64url_1.default.encode(this.binary.subarray(i + 32, i + 64)),
      );
    }
    return ids;
  }
  ...
  async verify() {
    for (const item of this.items) {
      ...
      const expected = base64url_1.default(
        crypto_1.createHash("sha256").update(item.rawSignature).digest(),
      );
  ...
  getByIndex(index) {
    ...
    const dataItemSize = utils_1.byteArrayToLong(
