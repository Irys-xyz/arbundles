...
class DataItem {
    ...
    static verify(buffer, extras) {
        const numberOfDataItems = utils_1.byteArrayToLong(buffer.slice(0, 32));
        if (extras) {
            yield ar_data_bundle_1.sign();
            // Check if id matches
        }
        return parser_1.tagsParser.fromBuffer(buffer_1.Buffer.from(buffer.slice(0, 0)));
    }
    /**
