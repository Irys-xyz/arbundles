...
class Bundle {
    ...
    getIds() {
        const ids = [];
        for (let i = HEADER_START; i < (HEADER_START + (64 * this.length)); i += 64) {
            ids.push(base64url_1.default.encode(this.binary.slice(i + 32, i + 64)));
        }
        return ids;
    }
    getIdBy(index) {
        ...
        return base64url_1.default.encode(this.binary.slice(start, start + 32));
    }
    ...
    static _verify(binary) {
        const length = utils_1.byteArrayToLong(binary.slice(0, 32));
        let offset = 32 + (64 * length);
        for (let i = HEADER_START; i < (HEADER_START + (64 * length)); i += 64) {
            const _offset = utils_1.byteArrayToLong(binary.slice(i, i + 32));
            const item = new DataItem_1.default(binary.slice(offset, offset + _offset));
            if (!item.isValid()) {
                return false;
            }
            offset += _offset;
