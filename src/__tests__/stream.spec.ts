...
describe("streamSigner", () => {
  describe("given we have a stream", () => {
    it("should return a signed stream", async () => {
      writeFileSync("test.txt", Buffer.from("102030405060708090a", "hex"));
      const signer = signerTestVariations[0].signer;
      const signedStream = await streamSigner(createReadStream("test.txt"), createReadStream("test.txt"), signer);
      const data = new Promise<Buffer>((res, rej) => {
        const bufs = [];
        signedStream.on("data", (chunk) => bufs.push(chunk));
...
describe("readBytes", () => {
  describe("given we have a reader, a buffer and a length", () => {
    it("should read the bytes", async () => {
      writeFileSync("test.txt", Buffer.from("102030405060708090a0", "hex"));
      const reader = streamExportForTesting.getReader(createReadStream("test.txt"));
      const result = await streamExportForTesting.readBytes(reader, Buffer.allocUnsafe(0), 0);
