...
describe("DataItem", () => {
                    ...
                    describe.each(testDataVariations)("and with $description data", ({ data }) => {
                        ...
                        describe("given we use getStartOfData()", () => {
                            it("should return the start of data", async () => {
                                ...
                                //@ts-expect-error
                                const numberOfTagBytesArray = dataItem.binary.subarray(
                                    tagsStart + 8,
                                    tagsStart + 16,
                                );
                        ...
                        describe("given we access rawTarget", () => {
                            it("should return the raw target", async () => {
                                expect(dataItem.rawTarget).toEqual(Buffer.from(target ?? ""));
                            });
                        });

                        describe("given we access rawTags", () => {
                            it("should return the raw tags", async () => {
                                expect(dataItem.rawTags).toEqual(serializeTags(tags ?? []));
                            });
                        });
    ...
    describe("given we have a known Dataitem", () => {
        ...
        const tags = [{ name: "tag1", value: "value1" }, { name: "tag2", value: "value2" }];
        ...
        describe("given we a DataItem with oversized tags", () => {
            describe("and we call isValid()", () => {
                it("should return false", async () => {
                    dataItem = createData(data, signer, { anchor, target, tags: [{ name: "a", value: "b" }, { name: "oversized", value: "oversized".repeat(4096) }] });
                    await dataItem.sign(signer);
                    expect(await dataItem.isValid()).toEqual(false);
                });
            });
